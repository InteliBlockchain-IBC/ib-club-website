...
const InnerApp = () => {
  ...
  const setupWalletConnection = async (address) => {
    try {
      ...
      setErrorMessage(t('errors.wallet_setup'));
    }
  };
  ...
  const connectWallet = async () => {
    ...
    try {
      if (!window.ethereum) {
        setErrorMessage(t('errors.metamask_missing'));
      ...
      if (error.code === 4001) {
        setErrorMessage(t('errors.user_cancelled'));
      } else {
        setErrorMessage(t('errors.wallet_connect'));
      }
    } finally {
      setIsConnecting(false);
    }
  ...
  const achievements = [
    {
      number: "10+",
      title: t('achievements.projects'),
      description: t('hero.subtitle'),
      icon: "fas fa-code"
    },
    {
      number: "30+",
      title: t('achievements.members'),
      description: 'Comunidade crescente de entusiastas',
      icon: "fas fa-users"
    },
    {
      number: "10+",
      title: t('achievements.partners'),
      description: 'Empresas e organizações colaborativas',
      icon: "fas fa-handshake"
    },
    {
      number: "20+",
      title: t('achievements.events'),
        ...
        <section className="hero">
          <div className="hero-content">
            <h1 className="hero-title">
              {t('hero.title_part1')}
              <span> {t('hero.title_span')} </span> <br />
              {t('hero.title_part2')}
            </h1>
              ...
              <div className="partners-carousel">
                <div className="partners-track">
                  {partners.map((partner, index) => (
                    partner.link ? (
                      <a key={`first-${index}`} href={partner.link} target="_blank" rel="noopener noreferrer" className="partner-card">
                  ...
                  {partners.map((partner, index) => (
                    partner.link ? (
                      <a key={`second-${index}`} href={partner.link} target="_blank" rel="noopener noreferrer" className="partner-card">
                        <div className="partner-logo">
                          <img src={partner.logo} alt={partner.name} />
                        </div>
                      </a>
      ...
      <nav className="navbar">
        <div className="navbar-content">
          ...
          <div className="nav-links-center">
            <button onClick={() => handleNavigation('home')} className={`nav-link ${currentPage === 'home' ? 'active' : ''}`}>{t('nav.home')}</button>
            <button onClick={() => handleNavigation('projetos')} className={`nav-link ${currentPage === 'projetos' ? 'active' : ''}`}>{t('nav.projects')}</button>
            <button onClick={() => handleNavigation('calendario')} className={`nav-link ${currentPage === 'calendario' ? 'active' : ''}`}>{t('nav.calendar')}</button>
            <button onClick={() => handleNavigation('eventos')} className={`nav-link ${currentPage === 'eventos' ? 'active' : ''}`}>{t('nav.memories')}</button>
            <button onClick={() => handleNavigation('sobre')} className={`nav-link ${currentPage === 'sobre' ? 'active' : ''}`}>{t('nav.about')}</button>
          </div>
          ...
          <div className="nav-links-right">
            ...
            <div className="lang-switch">
              <button onClick={() => changeLang('pt')} className={`lang-btn ${lang === 'pt' ? 'active' : ''}`}>PT</button>
              <button onClick={() => changeLang('en')} className={`lang-btn ${lang === 'en' ? 'active' : ''}`}>EN</button>
            </div>
            ...
            <div className="wallet-section">
              ...
              {isWalletConnected ? (
                <div className="wallet-info" onClick={() => setShowWalletModal(true)}>
                  ...
                  <button className="connect-wallet connected"><i className="fas fa-wallet"></i></button>
                </div>
              ) : (
                <button className={`connect-wallet ${isConnecting ? 'connecting' : ''}`} onClick={connectWallet} disabled={isConnecting}>
                  {isConnecting ? (<><i className="fas fa-spinner fa-spin"></i>{t('wallet.connecting')}</>) : (<><i className="fas fa-wallet"></i>{t('wallet.connect')}</>) }
                </button>
              )}
            </div>
          </div>
        ...
        <div className="wallet-modal-overlay" onClick={() => setShowWalletModal(false)}>
          <div className="wallet-modal" onClick={(e) => e.stopPropagation()}>
            <div className="wallet-modal-header">
              <h3>{t('wallet.modal_title')}</h3>
              <button className="wallet-modal-close" onClick={() => setShowWalletModal(false)}><i className="fas fa-times"></i></button>
            </div>
            ...
            <div className="wallet-modal-content">
              <div className="wallet-info-item">
                ...
                <div className="wallet-address-container">
                  ...
                  <button className="copy-button" onClick={() => copyToClipboard(walletAddress)} title={t('wallet.copy')}>
              ...
              <div className="wallet-info-item">
                ...
                <span className="wallet-balance-full">{parseFloat(walletBalance).toFixed(6)} {networkInfo?.symbol || 'ETH'}</span>
              </div>
            ...
            <div className="wallet-modal-actions">
              <button className="conquistas-button" onClick={() => handleNavigation('conquistas')}><i className="fas fa-trophy"></i>{t('wallet.viewAchievements')}</button>
              <button className="disconnect-button" onClick={disconnectWallet}><i className="fas fa-sign-out-alt"></i>{t('wallet.disconnect')}</button>
            </div>
          </div>
        </div>
      )}
